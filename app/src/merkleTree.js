...
export { checkProof, checkProofOrdered, merkleRoot, checkProofSolidityFactory, checkProofOrderedSolidityFactory };
...
function combinedHash(first, second, preserveOrder) {
	...
	if (preserveOrder) {
		return toBuffer(web3.utils.soliditySha3(bufJoin(first, second)));
	} else {
		return toBuffer(web3.utils.soliditySha3(bufSortJoin(first, second)));
	}
}
