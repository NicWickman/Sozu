...
const ansCids = cids.map((x, i) => [i, x, i % 2]);
...
contract('Job', (accounts) => {
	...
	it('should prove the first cid is legitimate', async () => {
		const merkleTree = new MerkleTree(cids);
		const root = merkleTree.getHexRoot();
		const proof = merkleTree.getHexProof(cids[0]);
		const leaf = keccak256(ansCids[0]);

		let instance = await Job.deployed();
		let proven = await instance.verifyProof(proof, root, leaf);
		console.log(proven);
	});
});
